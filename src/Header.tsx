  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled
          ? 'bg-white/90 border-b border-brand-100/50 py-3'
          : 'bg-transparent py-5'
      }`}
    >
      <div className="container mx-auto px-4 max-w-7xl">
        <div className="flex justify-between items-center">
          <a href="#" className="text-xl font-extrabold text-brand-900 tracking-tight">
          ...
          <nav className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="text-sm font-semibold text-brand-500 hover:text-brand-900 transition-colors"
              >
                {link.name}
              </a>
            ))}
            <a
              ...
              className="inline-flex items-center gap-2 bg-[#25D366] text-white px-5 py-2.5 rounded-lg text-sm font-semibold hover:bg-[#128C7E] transition-all active:scale-[0.98]"
          ...
          <button
            className="md:hidden text-brand-900 p-3 min-w-[44px] min-h-[44px] flex items-center justify-center"
        ...
        {mobileMenuOpen && (
          <div className="md:hidden absolute top-full left-0 right-0 bg-white border-b border-brand-100 p-6 animate-in slide-in-from-top-3 duration-200">
            <nav className="flex flex-col" aria-label="Navegação principal">
              {navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
                  className="text-base font-semibold text-brand-700 min-h-[44px] flex items-center"
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center justify-center gap-3 bg-[#25D366] text-white py-4 rounded-lg font-semibold text-base"
              >
