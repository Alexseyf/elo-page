  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled
          ? 'bg-white/95 backdrop-blur-md border-b border-brand-100/70 py-3 shadow-subtle'
          : 'bg-transparent py-5'
      }`}
    >
      <div className="container mx-auto px-4 max-w-7xl">
        <div className="flex justify-between items-center">
          <a href="#" className="text-xl font-extrabold text-brand-900 tracking-tight flex items-center gap-1.5">
            <span>Elo</span>
          ...
          <nav className="hidden lg:flex items-center gap-7">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="text-sm font-bold text-brand-600 hover:text-brand-900 transition-colors"
              >
                {link.name}
              </a>
            ))}
            ...
            <nav className="flex flex-col space-y-1" aria-label="Navegação principal">
              {navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
                  className="text-base font-bold text-brand-700 min-h-[44px] flex items-center hover:text-brand-900"
